...
**What the request count costs depends on whose pool it is.** vLLM is priced so every concurrent request can hold the full context at once. SGLang serves all of them from one pool and treats `--max-running-requests` as an admission limit, so a slot costs the state above and no pool at all — the pool is sized to hold one full-length request, and a request whose input exceeds it is refused with HTTP 400 rather than the server failing to start. Pricing it the strict way made configurations impossible that the upstream recipe runs daily: its own 4-request profile holds a 413,460-token pool, which is 1.6 full-length requests, not 4. Raise the fraction above the computed minimum to buy a deeper pool; the panel says so on any run with more than one request.
